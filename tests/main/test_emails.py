...
class SendEmailTaskIntegrationTestCase(TestCase):
    """Integration tests for send_email_task with more realistic scenarios."""

    @patch("resend.Emails.send")
    @override_settings(
        ENABLE_EMAILS=True,
        DEBUG=False,
        DEFAULT_FROM_EMAIL="notifications@myapp.com",
        RESEND_API_KEY="re_123456789",
    )
    def test_realistic_email_scenario(self, mock_send):
        """Test with realistic email content and multiple recipients."""
        # Arrange
        mock_send.return_value = MagicMock()

        realistic_subject = "Welcome to Our Platform!"
        realistic_html = """
        <html>
            <body>
                <h1>Welcome!</h1>
        ...
        send_email_task(
            ...
            message=realistic_html,
            recipient_list=realistic_recipients,
        )

        # Assert
        expected_params = {
            "from": "notifications@myapp.com",
            "to": realistic_recipients,
            "subject": realistic_subject,
            "html": realistic_html,
        }
        mock_send.assert_called_once_with(expected_params)
